...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	WriteBuffer[0] = (u8)(reg+2);
	WriteBuffer[1] = (u8)(val);
	ReadBuffer[0] = 0;
	ReadBuffer[1] = 0;


	return_code = XSpi_Transfer(&SpiInstance, WriteBuffer, ReadBuffer, 2);

	if(return_code != 0 )
		xil_printf ("read buffer error");
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	return_code = XSpi_Transfer(&SpiInstance, WriteBuffer, ReadBuffer, nbytes+1);

	if(return_code !=0)
		xil_printf ("read buffer multibyte error");
